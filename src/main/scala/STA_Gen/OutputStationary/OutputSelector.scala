...
class OutputSelector(val arrayRow: Int, val arrayCol : Int, val blockRow : Int, val blockCol : Int) extends Module {
  ...
  val numberOfSignals: Int = arrayCol + arrayRow - 1
  val numberOfOutputs: Int = numberOfSignals * blockRow * blockCol
  ...
  for( i <- 0 until numberOfSignals)
    for( j <- 0 until blockRow * blockCol)
      io.output(j + i * blockRow * blockCol) := RegNext(Mux(io.selectionSignal(i), io.input(j + i * blockRow * blockCol), 0.S), 0.S)
